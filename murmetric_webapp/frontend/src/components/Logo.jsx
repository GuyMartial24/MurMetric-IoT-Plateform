...
// Couleurs calées sur le bandeau bleu du header (seul endroit où ce
// composant est utilisé, cf. App.jsx `.entete-accent`) — pas de tokens ici
// puisque ce n'est pas un composant shadcn générique réutilisé ailleurs.
  ...
  return (
    ...
    >
      <svg width={taille} height={taille} viewBox="0 0 64 64" aria-hidden="true">
        ...
        />
        <circle cx="46" cy="10" r="4.5" fill="#ffffff" />
      </svg>
      <div style={{ lineHeight: 1.15 }}>
        <div style={{ fontWeight: 700, fontSize: `${taille * 0.5}px`, color: "#ffffff" }}>MurMetric</div>
        <div style={{ fontSize: `${taille * 0.26}px`, color: "#bfdbfe", letterSpacing: "0.03em" }}>by FRD-CODEM</div>
      </div>
    </div>
  );
}
