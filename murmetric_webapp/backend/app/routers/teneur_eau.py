...
from ..influx import MESURE_TENEUR_EAU, delete_points, echap_field_str, echap_tag, flux_escape, query_api, write_point
...
class SaisieTeneurEau(BaseModel):
    mur: str
    couche: str
    valeur_pourcent: float = Field(..., description="Arrondi serveur à 2 décimales — cf. correction précision du backfill historique")
    commentaire: str = ""
...
def _construire_ligne(utilisateur: dict, mur: str, couche: str, valeur: float, commentaire: str, date_mesure: datetime) -> str:
    tags = (
...
def creer(saisie: SaisieTeneurEau, utilisateur: dict = Depends(utilisateur_courant)) -> dict:
    ...
    ligne = _construire_ligne(utilisateur, saisie.mur, saisie.couche, saisie.valeur_pourcent, saisie.commentaire, date_mesure)
    try:
        write_point(ligne)
...
@router.put("")
def corriger(correction: CorrectionTeneurEau, utilisateur: dict = Depends(utilisateur_courant)) -> dict:
    ...
    if not identite_inchangee:
        ...
        try:
            delete_points(predicat, correction.date_mesure_original - marge, correction.date_mesure_original + marge)
        except Exception as exc:
            raise HTTPException(status_code=502, detail=f"Suppression du point original échouée : {exc}") from exc

    ligne = _construire_ligne(utilisateur, correction.mur, correction.couche, correction.valeur_pourcent, correction.commentaire, correction.date_mesure)
    try:
        write_point(ligne)
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"Écriture InfluxDB échouée : {exc}") from exc
